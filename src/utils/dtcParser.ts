...
export const parseDtcMaskGroups = (textContent: string): ParsedGroup[] => {
  ...
  // Sprawdź ile DTC_MASK jest w oczyszczonym tekście
  ...
  while (currentPos < workingText.length) {
    ...
    console.log(`Group preview: "${groupText.substring(0, 80)}"`);
...
const parseGroupContent = (groupText: string, groupNumber: number): ParsedGroup | null => {
  ...
  if (!dtcMatch) {
    console.log(`No DTC pattern found in group ${groupNumber}, text: "${groupText.substring(0, 50)}"`);
    return null;
  }
